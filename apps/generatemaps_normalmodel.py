...
trained_normal_maps_path = "{0}/FSS/trained_normal_maps".format(opt.home_dir)

os.makedirs(trained_normal_maps_path, exist_ok=True)
...
def generate_maps(opt):
    ...
    F_modelnormal_path = "{0}/FSS/apps/checkpoints/Date_09_Jul_22_Time_13_56_45/netF_model_state_dict_epoch34.pickle".format(opt.home_dir)
    B_modelnormal_path = "{0}/FSS/apps/checkpoints/Date_09_Jul_22_Time_13_56_45/netB_model_state_dict_epoch34.pickle".format(opt.home_dir)
    ...
    with torch.no_grad():
        ...
        for dataset_type, data_loader in data_loader_list:
            ...
            _iterable = enumerate(data_loader)

            for train_idx, train_data in _iterable:
                print("batch {}".format(train_idx) )

                train_data_list = [train_data]

                for _index, train_data in enumerate(train_data_list):
                    ...
                    mask_high_res_array = train_data['mask'].cpu().numpy() # Shape of [Batch_size, 1, Height, Width]
                    ...
                    for i in range(opt.batch_size):
                        ...
                        sparse_matrix = sparse.load_npz(save_normalmapF_path).todense()
                        netF_normalmap_np = np.asarray(sparse_matrix) 
                        netF_normalmap_np = np.reshape(netF_normalmap_np, [3, HIGH_RES_SIZE, HIGH_RES_SIZE] )

                        sparse_matrix = sparse.load_npz(save_normalmapB_path).todense()
                        netB_normalmap_np = np.asarray(sparse_matrix) 
                        netB_normalmap_np = np.reshape(netB_normalmap_np, [3, HIGH_RES_SIZE, HIGH_RES_SIZE] )
                        """
